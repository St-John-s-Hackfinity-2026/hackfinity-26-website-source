...
The public GitHub Pages release `9cad4a7` was verified after the final school-brand cleanup. It retains the live squad count, confirms St. John’s School, Anchal as the organizer, removes Hack Club wording and the public Google Sheets registration notice, and exposes no organizer-hub link in the public footer. The footer now contains only `© 2026 St. John's School, Anchal.`

The local cross-device timeline repair replaces stale cyan touch-state declarations with the school red/yellow/white palette, positions the active fill layer behind the full card content, and gives each 01–05 numeral a protected outline-and-fill treatment. Desktop active-state inspection confirms the red/yellow fill and the complete 01 numeral; the same protected width, padding, and active-state rule apply to 02–05 at mobile and desktop breakpoints.

GitHub Pages release `d80d3ee` (`Fix timeline fill palette and stage numerals`) deployed successfully. The deployed public page and live squad count were rechecked. The repaired desktop active state was inspected directly. The local narrow-screen code path is covered by the dedicated CSS regression checks; an independent headless capture could not render the authenticated local preview, so final touch-device visual confirmation remains an organizer-side follow-up.

On 14 August 2026, the school Apps Script endpoint was verified to return the `registrations` JSONP action. It exposes only the intended privacy-safe roster fields: squad name, participation format, project title, battle track, member count, and submitted time. The public organizer command center was opened at the static-compatible URL and began loading this roster; private names, school details, emails, phone numbers, and project descriptions remain absent from the endpoint and protected in Google Sheets.

GitHub Pages release `2298ad1` (`Apply St Johns palette to organizer panel`) completed successfully. The public organizer command center now uses the same school palette across the sidebar, hero, metrics, action cards, protective notice, roster search, table, and record controls: dark red structure, yellow emphasis, and white content. During the cache-busted visual check, the previously verified Apps Script count briefly displayed its existing reconnect state; no data-access code changed in this color-only release.

The Apps Script count endpoint was checked directly after that temporary state and returned `ok: true` with a count of `6`, confirming that the color-only organizer release did not affect the registration service.

The blank live-counter report was diagnosed on the public page: the Apps Script JSONP count does respond, but its initial asynchronous round trip can leave the hero showing its loading glyph before the response arrives. GitHub Pages release `8452f09` now preserves the last confirmed count in the browser and displays it while a refresh is pending. A fresh public-page check confirmed the endpoint returns and the hero resolves to the current count (`07` at the time of verification).
